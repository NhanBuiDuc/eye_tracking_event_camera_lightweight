        ...
        for i in range(self.num_bins):
            ...
            if self.focal_loss:
                ...
                label_matrix = torch.zeros(
                    (self.S, self.S, self.C + 5 * self.B), dtype=torch.float # 5: x, y, w, h confidence
                )
                row, column = int(self.S * y_norm), int(self.S * x_norm)

                # label 
                label_matrix[row, column, self.C] = 1  # obj confidence
                label_matrix[
                    row, column, (self.C + 1) : (self.C + 1 + 4)
                ] = box_coordinates  # box coord

                # for b in range(1, self.B):
                #     # Placeholder for additional bounding box coordinates
                #     # Ideally, these should be different if you have different bounding boxes
                #     box_coordinates_b = box_coordinates  # Using the same coordinates here for simplicity

                #     # Calculate the offset for the b-th bounding box
                #     offset = self.C + b * 5

                #     # Set the object confidence for the b-th bounding box
                #     label_matrix[row, column, offset] = 1  # object confidence
                #     label_matrix[
                #         row, column, (offset + 1):(offset + 5)
                #     ] = box_coordinates_b  # box coordinates for the b-th bounding box
